...
export default function useKMarketHook() {
  ...
  // 创建k线图
  function createKLineChart(isExtension = true, isAddCustom = false) {
    ...
    // 默认渲染
    if (isExtension) {
      chart.createIndicator('MACD', true, { id: 'MACD', height: 100 })
      chart.createIndicator('MA', true, { id: 'candle_pane' })
    }
    return chart
  }

  function updateChartHeight(chart, klineRef, subCount, indicatorHeight) {
    if (!baseHeight) {
      // 只获取一次高度
      baseHeight = parseFloat(getComputedStyle(klineRef).height)
      console.log('只获取一次高度', baseHeight)
    }
    const subHeight = indicatorHeight // 添加的指标的高度
    const newHeight = subCount * subHeight
    klineRef.style.height = baseHeight + newHeight + 'px'
    chart.resize()
  }
