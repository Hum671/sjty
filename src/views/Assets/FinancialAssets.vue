...
const { financialBalance, balanceLoading } = storeToRefs(userStore)
...
watch(hasDataChange, () => {
  ...
  // 2、排序
  financialBalance.value = financialBalance.value.sort((a, b) => b.rateNumber - a.rateNumber)

  const total = financialBalance.value.reduce((accumulator, currentValue) => {
    return accumulator + Number(currentValue.rateNumber)
  }, 0)

  totalAssets.value = Number(formatNumber(total, 8))
  showAssetList.value = financialBalance.value
