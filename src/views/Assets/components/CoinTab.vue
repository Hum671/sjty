...
watch(coinList, (val) => {
  ...
  if (!coinList.value || !userCoinBalance.value) {
    return
  }

  const newAssetsList = deepCopy(unref(assetsList.value))
  userCoinBalance.value.forEach((val) => {
    const findAsset = newAssetsList.find((item) => item.name === val.coin)
    if (findAsset) {
      findAsset.number = Number(val?.number ?? '0')
      findAsset.occNumber = Number(val?.occNumber ?? '0')

      if (val.coin === 'USDT') {
        findAsset.rateNumber = findAsset.number
        return
      }

      const find = coinList.value.find((item) => item.name.toLowerCase() === val.coin.toLowerCase())
      findAsset.rateNumber = Number(
        formatNumber(findAsset.number * (find?.data?.tick?.lastPrice ?? 0), 8),
      )
    }
  })
