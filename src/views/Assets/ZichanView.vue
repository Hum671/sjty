...
const { userBalance, balanceLoading } = storeToRefs(userStore)
...
watch(hasDataChange, () => {
  ...
  // 2、排序
  userBalance.value = userBalance.value.sort((a, b) => b.rateNumber - a.rateNumber)

  const total = userBalance.value.reduce((accumulator, currentValue) => {
    return accumulator + Number(currentValue.rateNumber)
  }, 0)

  totalAssets.value = Number(formatNumber(total, 8))
  showAssetList.value = userBalance.value
